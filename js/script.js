...
function setupEventListeners() {
    ...
    addIndicatorBtn.addEventListener('click', () => { const name = newIndicatorInput.value.trim(); if (name) { const newId = appState.indicators.length > 0 ? Math.max(...appState.indicators.map(i => i.id)) + 1 : 1; const frequency = newIndicatorFrequency.value; appState.indicators.push({ id: newId, name: name, frequency: frequency }); newIndicatorInput.value = ''; savePlannerData(); renderPlanner(); }});
...
function renderWeeklyView() {
    ...
    const weekEndDate = new Date(weekStartDate);
    weekEndDate.setDate(weekEndDate.getDate() + 7);
    const scheduledTasks = calculateScheduledTimes(tasks, weekStartDate, weekEndDate);
    ...
    if (typeof scheduledTasks !== 'undefined' && scheduledTasks.length > 0) {
...
function renderDailyView() {
    ...
    const scheduledTasks = calculateScheduledTimes(tasks, dayStart, dayEnd);
